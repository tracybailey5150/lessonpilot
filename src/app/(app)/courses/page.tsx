...
export default function CoursesPage() {
  ...
  async function deleteCourse(courseId: string) {
    ...
    await supabase.from('curriculum_units').delete().eq('course_id', courseId)
    await supabase.from('source_documents').delete().eq('course_id', courseId)
  ...
  const s = {
    ...
    card: (isConfirm: boolean) => ({ background: '#0C1220', border: isConfirm ? '1px solid rgba(239,68,68,0.4)' : '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', padding: '24px', display: 'flex', flexDirection: 'column' as const, gap: '12px' } as React.CSSProperties),
    ...
    levelBadge: (level: string) => ({ display: 'inline-block', background: `${levelColor(level)}18`, color: levelColor(level), borderRadius: '6px', padding: '3px 10px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.05em' } as React.CSSProperties),
    ...
    deleteBtn: { background: 'none', border: 'none', color: '#475569', cursor: 'pointer', padding: '4px', fontSize: '16px', lineHeight: 1, borderRadius: '4px' } as React.CSSProperties,
  }
        ...
        {courses.length === 0 ? (
          ...
          <div style={s.grid}>
            {courses.map(course => {
              ...
              const isConfirm = confirmDelete === course.id
              return (
                <div key={course.id} style={s.card(isConfirm)}>
                  {isConfirm ? (
                    <>
                      <div style={{ fontSize: '14px', fontWeight: 600, color: '#F87171' }}>Delete &ldquo;{course.title}&rdquo;?</div>
                      <div style={{ fontSize: '12px', color: '#64748B' }}>This will permanently delete the course and all its lessons.</div>
                      <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                        <button
                          onClick={() => deleteCourse(course.id)}
                          disabled={deletingId === course.id}
                          style={{ flex: 1, background: '#EF4444', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}
                        >
                          {deletingId === course.id ? 'Deleting...' : 'Yes, delete'}
                        </button>
                        <button
                          onClick={() => setConfirmDelete(null)}
                          style={{ flex: 1, background: 'rgba(255,255,255,0.06)', color: '#94A3B8', border: 'none', borderRadius: '8px', padding: '10px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}
                        >
                          Cancel
                        </button>
                      </div>
